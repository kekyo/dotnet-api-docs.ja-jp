### <a name="aspnet-mvc-now-escapes-spaces-in-strings-passed-in-via-route-parameters"></a>ASP.NET MVC ルート パラメーター経由で渡される文字列にスペースをエスケープするようになりました
...
|提案される解決策|ルートからの文字列パラメーターをエスケープ解除するように、コードを更新する必要があります。 元の URI が必要な場合にアクセスできる、<xref:System.Net.HttpWebRequest.RequestUri>です。OriginalString API です。|
|スコープ|マイナー|
|Version|4.5.2|
|型|ランタイム|
